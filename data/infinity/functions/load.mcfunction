scoreboard objectives add mobCount dummy
scoreboard objectives add killLimit dummy
forceload add 642 -617
forceload add -628 608
forceload add 680 632
forceload add -586 -654
forceload add 623 627
forceload add -626 -608
forceload add 10273 -9864
forceload add -10042 9741
forceload add 10888 10123
forceload add -9375 -10457
forceload add 9973 10035
forceload add -10019 -9712
scoreboard objectives add initCheck dummy
execute unless score global initCheck matches 1 run function infinity:init_once