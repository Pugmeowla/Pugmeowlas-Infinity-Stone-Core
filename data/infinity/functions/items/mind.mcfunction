...
execute as @a at @s run tag @e[type=item,distance=..8,nbt={Item:{id:"infinity:mind_stone"}}] add mind_stone_found

# act on the tag from here on -- no more NBT comparisons
execute as @e[tag=mind_stone_found] at @s run summon armor_stand ~ ~ ~ {DisabledSlots:0,ShowArms:1b,Invisible:1b,NoBasePlate:1b,Small:1b,Invulnerable:1b,Tags:["mind_stone_ground"],Passengers:[{id:"minecraft:interaction",width:0.4f,height:-0.7f,Tags:["mind_stone_ground_interaction"]}]}
execute as @e[tag=mind_stone_found] at @s run kill @s
...
tag @e[tag=mind_stone_found] remove mind_stone_found
