...
execute as @a at @s run tag @e[type=item,distance=..8,nbt={Item:{id:"infinity:soul_stone"}}] add soul_stone_found

# act on the tag from here on -- no more NBT comparisons
execute as @e[tag=soul_stone_found] at @s run summon armor_stand ~ ~ ~ {DisabledSlots:0,ShowArms:1b,Invisible:1b,NoBasePlate:1b,Small:1b,Invulnerable:1b,Tags:["soul_stone_ground"],Passengers:[{id:"minecraft:interaction",width:0.4f,height:-0.7f,Tags:["soul_stone_ground_interaction"]}]}
execute as @e[tag=soul_stone_found] at @s run kill @s

execute as @e[type=armor_stand,tag=soul_stone_ground] at @s run superpower add infinity:soul_stone_ground @s
...
tag @e[tag=soul_stone_found] remove soul_stone_found
